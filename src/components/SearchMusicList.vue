...
const fetchData = async (Keyword) => {
  try {
    const res = await GetSearchData({ keyword: Keyword, type: 1000, limit: 10 })
    items.value = (res.result.playlists || []).map((m: any) => ({
      image: m.coverImgUrl,
      title: m.name,
      id: m.id,
    }))
...
const TurnIn = (item: Item) => {
  router.push({ name: 'searchmusiclist', params: { id: item.id } })
}
