...
async function loadWholelistData(loadMore = false) {
  ...
  try {
    ...
    if (res.result && res.result.songs && res.result.songs.length < limit) {
      hasMore.value = false
    }
    ...
    const uniqueNewItems = addsongs.value.filter((item: SongItem) => !existingIds.has(item.id))

    if (uniqueNewItems.length === 0 && addsongs.value.length > 0) {
      // 如果获取到的全是重复的，可能到底了
      hasMore.value = false
    } else {
